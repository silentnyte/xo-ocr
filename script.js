...
var featureImgs = {
  ...
  featureFirePercent: new Image(),
  featureMeleePercent: new Image(),
  featurePassthroughPercent: new Image(),
};
featureImgs.featureBlastPercent.src =
...
featureImgs.featureBulletPercent.src =
...
featureImgs.featureFirePercent.src =
...
featureImgs.featureMeleePercent.src =
...
featureImgs.featurePassthroughPercent.src =
  ...
  for (let l in data.lines) {
    ...
    if (txt.length > 0) {
      ...
      if (txt.match(/Increases vehicle durability/i)) {
        ...
        document.getElementById("increasesDurability").value = val;
        ...
        var txtsplit = txt.split(" ");
        var val = txtsplit[txtsplit.length - 1].replace(/[^-\d]/g, "");
        document.getElementById("increasesReputationPercent").value = val;
      } else if (txt.match(/Top speed/i)) {
        addField("form_stats", "topSpeed", "number", "Top speed", false);
        var txtsplit = txt.split(" ");
        var val = txtsplit[3].replace(/[^-\d]/g, "");
        document.getElementById("topSpeed").value = val;
      } else if (
...
function addField(parent, name, type, label, isDec) {
  ...
  var frow = container.appendChild(document.createElement("div"));
  frow.className = "form-group form-row";
  var lbl = frow.appendChild(document.createElement("label"));
  lbl.className = "col-sm-2 col-form-label col-form-label-sm";
  lbl.textContent = label;
  var indiv = frow.appendChild(document.createElement("div"));
  indiv.className = "col-sm-5";
  if (type == "text" || type == "number") {
    var input = indiv.appendChild(document.createElement("input"));
    input.id = name;
    input.name = name;
    input.type = type;
    if(isDec) { input.step = "0.1"; }
    input.className = "form-control";
    var currentin = frow.appendChild(document.createElement("label"));
    currentin.id = 'current-' + name;
    currentin.className = "col-sm-5 col-form-label col-form-label-sm";
    if(xoDB_data != null) {
      currentin.textContent = xoDB_data[name];
    }
    } else {
    var texta = indiv.appendChild(document.createElement("textarea"));
    texta.id = name;
    texta.name = name;
    texta.cols = "50";
    texta.rows = "4";
    texta.className = "form-control form-control-sm";
    var currentdiv = frow.appendChild(document.createElement("div"));
